...
export default function Planner({ topics, sessions, completedDays }: { topics: Topic[]; sessions: Session[]; completedDays: CompletedDay[] }) {
  ...
  const [topicDrag, setTopicDrag] = useState<{id:number;edge:"start"|"end"}|null>(null);
  ...
  const dropOnDay = async (date:string) => { if(topicDrag){await resizeTopicBoundary(topicDrag.id,date,topicDrag.edge);setTopicDrag(null);return} await dropEvent(date); };
      ...
      <div className="calendar-shell">
        ...
        <div className="calendar-grid">{days.map((day,i)=>{
          ...
          return <button className={`day ${faded?"faded":""} ${dragging||topicDrag?"drop-ready":""}`} key={i} onClick={()=>openSession(day)} onDragOver={e=>{if(dragging||topicDrag)e.preventDefault()}} onDrop={e=>{e.preventDefault();e.stopPropagation();void dropOnDay(key)}}>
            ...
            <div className="coverage-bands">{covering.slice(0,2).map(t=>{const starts=t.startDate===key,ends=t.targetDate===key,weekStart=day.getDay()===1,isDone=completedDays.some(d=>d.topicId===t.id&&d.date===key);return <div key={t.id} className={`coverage ${starts?"coverage-start":""} ${ends?"coverage-end":""} ${isDone?"coverage-done":""}`} style={{"--accent":colors[t.color]||colors.violet} as React.CSSProperties} title={`${t.title}: ${isDone?"done":"not completed"}`} onClick={e=>{e.stopPropagation();openTopicDay(t,key)}}>{starts&&<i className="coverage-handle coverage-handle-start" draggable onClick={e=>e.stopPropagation()} onDragStart={e=>{e.stopPropagation();setTopicDrag({id:t.id,edge:"start"});e.dataTransfer.effectAllowed="move"}} onDragEnd={()=>setTopicDrag(null)} title="Drag to change start date"/>}<b>{isDone&&<Check size={10}/>} {(starts||weekStart||i===0)?t.title:""}</b>{ends&&<i className="coverage-handle coverage-handle-end" draggable onClick={e=>e.stopPropagation()} onDragStart={e=>{e.stopPropagation();setTopicDrag({id:t.id,edge:"end"});e.dataTransfer.effectAllowed="move"}} onDragEnd={()=>setTopicDrag(null)} title="Drag to change end date"/>}</div>})}</div>
